...
class CrossEntropy(nn.Module):
	...
	def cuda(self, **kwargs):
		network = super(CrossEntropy, self).cuda(**kwargs)
		self.gpu = True
		self.cuda_kwargs = kwargs
		return network

	def cpu(self):
		network = super(CrossEntropy, self).cpu()
		self.gpu = False
		self.cuda_kwargs = dict()
		return network
	...
	def forward(self, label_tensor, feature_a, feature_b):
		...
		for i in range(batch_size):
			true_label_cnt = 0
			tmp_loss = torch.zeros(1, dtype=torch.float32)
			if self.gpu:
				tmp_loss = tmp_loss.cuda(**self.cuda_kwargs)
			for j in range(batch_size):
				if label_tensor[i]==label_tensor[j]:
					tmp_loss -= torch.log(cos_score[j])
					true_label_cnt += 1
